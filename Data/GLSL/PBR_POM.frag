...
void main()
{
    ...
    // GRAZING ANGLE FADE
    float grazingFade = clamp(Vtangent.z * 10.0, 0.0, 1.0);
    float heightScale_val = heightScale * grazingFade;

    // CALCULATE DISPLACEMENT
    vec2 displacement =
        (Vtangent.xy / max(Vtangent.z, 0.001)) * heightScale_val;
    ...
    {
        int maximumLayers = max(layers, 1);
        int minimumLayers = min(maximumLayers, 8);
    ...
    // TEXTURE SAMPLING
    vec3 albedo = pow(textureGrad(albedoTexture, uv, baseUvDx, baseUvDy).rgb, vec3(2.2));
	...
	// Editor exposure boost.
	finalColor *= 1.45;

	// Editor exposure boost.
	// 1.0 = unchanged, 1.4 to 2.0 is usually good for this viewport.
	float exposure = 1.6;
	finalColor *= exposure;
